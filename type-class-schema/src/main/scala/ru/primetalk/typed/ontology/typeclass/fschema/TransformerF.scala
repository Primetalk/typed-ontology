...
trait TransformerF[F[+_], Source, -SourceV, Dest, +DestV] extends Function1[SourceV, F[DestV]]:
  def apply(source: SourceV): F[DestV]

trait ProjectionTransformerLowPriority2:

//  import RecordTupleValue.prepend
//
//  given nonEmptyDestHeadTransformer[H, HV, Source <: Tuple, SV <: Tuple, Dest <: Tuple, DV <: Tuple](using
...
//  given nonEmptyHeadMatchTransformer[H, HV, Source <: Tuple, SV <: Tuple, Dest <: Tuple, DV <: Tuple](using
//    svts: RecordValueType[Source, SV],
//    svtd: RecordValueType[Dest, DV],
//    hvt: SchemaValueType[H, HV],
//    proj: Transformer[Source, RecordTupleValue[Source, SV], Dest, RecordTupleValue[Dest, DV]]
//  ): Transformer[H *: Source, RecordTupleValue[H *: Source, HV *: SV], H *: Dest, RecordTupleValue[H *: Dest, HV *: DV]] = { case Prepend(head, tail) =>
//    proj(tail).prepend(head)
//  }
//
//  /** Another often seen case when source head is dropped. */
//  given missingColumnTransformer[H, HV, Source <: Tuple, SV <: Tuple, Dest <: Tuple, DV <: Tuple](using
//    svts: RecordValueType[Source, SV],
//    svtd: RecordValueType[Dest, DV],
//    hvt: SchemaValueType[H, HV],
//    proj: Transformer[Source, RecordTupleValue[Source, SV], Dest, RecordTupleValue[Dest, DV]]
//  ): Transformer[H *: Source, RecordTupleValue[H *: Source, HV *: SV], Dest, RecordTupleValue[Dest, DV]] = { case Prepend(_, tail) =>
//    proj(tail)
//  }
