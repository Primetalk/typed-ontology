...
trait TransUpdaterLowPriority2:
  ...
  given nonEmptyDestHeadTransUpdater[
      H,
      HV,
      Source <: Tuple,
      SV <: Tuple,
      ...
      transUpdater: TransUpdater[
        Source,
        RecordTupleValue[Source, SV],
        Dest,
        RecordTupleValue[Dest, DV]
      ]
  ): TransUpdater[Source, RecordTupleValue[Source, SV], H *: Dest, RecordTupleValue[
    H *: Dest,
    HV *: DV
  ]] =
    v => { case Prepend(_, tailDest) =>
...
trait TransUpdaterLowPriority:
  given missingChangeColumnTransUpdater[
      H,
      HV,
      Source <: Tuple,
      SV <: Tuple,
  ...
  ): TransUpdater[H *: Source, RecordTupleValue[H *: Source, HV *: SV], Dest, RecordTupleValue[
    Dest,
    DV
  ]] = { case Prepend(_, tail) =>
    transUpdater(tail)
  ...
  given missingDestColumnTransUpdater[
      H,
      HV,
      Source <: Tuple,
      SV <: Tuple,
    ...
    H *: Dest,
    HV *: DV
  ]] = changeV => { case Prepend(head, tail) =>
    head *: transUpdater(changeV)(tail).value
  }

  given nonEmptyHeadMatchTransUpdater[
      H,
      HV,
      Source <: Tuple,
      SV <: Tuple,
  ...
  ): TransUpdater[H *: Source, RecordTupleValue[H *: Source, HV *: SV], H *: Dest, RecordTupleValue[
    H *: Dest,
    HV *: DV
  ]] = {
    case Prepend(headChange, tailChange) => { case Prepend(_, tailDest) =>
...
object TransUpdater extends TransUpdaterLowPriority with TransUpdaterLowPriority2:
  given emptyChangeTransUpdater[Dest <: Tuple, DV <: Tuple](using
      svtd: RecordValueType[Dest, DV]
  ): TransUpdater[EmptyTuple, RecordTupleValue[EmptyTuple, EmptyTuple], Dest, RecordTupleValue[
    Dest,
    DV
  ]] = _ => identity
