...
/** Type-level annotation of a Tuple with it's RecordSchema. Elements are connected via Column[T],
  * SchemaValueType[C].
  */
opaque type RecordTupleValueF[F[+_], R <: Tuple, V <: Tuple] >: Tuple.Map[V, F] <: Tuple =
  Tuple.Map[V, F]
...
object RecordTupleValueF:
  ...
  inline given valueToRecordTupleValueF[F[+_], Schema <: Tuple, Value <: Tuple](using
      ev: SchemaValueType[Schema, Value]
  ): Conversion[Tuple.Map[Value, F], RecordTupleValueF[F, Schema, Value]] =
  ...
  given nonEmptyValidRecordTupleValueF[F[+_], C: Column: ValueOf, V, S <: Tuple, T <: Tuple](using
      svt: SchemaValueType[C, V]
  )(using ValidRecordTupleValueF[F, S, T]): ValidRecordTupleValueF[F, C *: S, F[V] *: T] =
    new ValidRecordTupleValueF
...
//    new SchemaValueType
  ...
  extension [F[+_], R <: Tuple, V <: Tuple](v: RecordTupleValueF[F, R, V])
    ...
    inline def prepend[H, HV](vh: F[HV])(using
        svt: SchemaValueType[H, HV]
    ): RecordTupleValueF[F, H *: R, HV *: V] =
      vh *: v.toTuple
    ...
    inline def project[Dest <: Tuple, DestV <: Tuple](dest: Dest)(using
        proj: ProjectorF[F, R, V, Dest, DestV]
    ): F[RecordTupleValueF[F, Dest, DestV]] =
      proj.apply(v)

  object Prepend:
    def unapply[F[+_], H, HV, R <: Tuple, V <: Tuple](
        r: RecordTupleValueF[F, H *: R, HV *: V]
    ): (F[HV], RecordTupleValueF[F, R, V]) =
      (r.head, r.tail)

  inline given dropGetter[F[
      +_
  ], OtherColumn, OtherColumnValue, Column, ColumnValue, Schema <: Tuple, SchemaValue <: Tuple](
      using getter: GetterF[F, Column, ColumnValue, RecordTupleValueF[F, Schema, SchemaValue]]
  ): GetterF[F, Column, ColumnValue, RecordTupleValueF[
    F,
    OtherColumn *: Schema,
  ...
  inline given headGetter[F[
      +_
  ]: Functor, Column, ColumnValue, Schema <: Tuple, SchemaValue <: Tuple]
      : GetterF[F, Column, ColumnValue, SchemaValue] =
    new GetterF[F, Column, ColumnValue, SchemaValue]:
      override def apply(rtc: Tuple.Map[SchemaValue, F]): F[ValueWithSchema[Column, ColumnValue]] =
        rtc match {
