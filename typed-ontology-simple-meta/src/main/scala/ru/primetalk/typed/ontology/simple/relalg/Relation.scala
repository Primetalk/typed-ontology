...
abstract class Relation[V[_]] extends ExprClassicDsl:
  ...
  transparent inline def crossProductFrom[R1 <: Relation[V]](r1: R1)(using
      FlatMap[V]
  ): Relation[V] =
  ...
  transparent inline def prependCalcColumn[P <: RecordProperty0](p: P)(inline f: Row => p.P)(using
      FlatMap[V]
  ) =
    import cats.syntax.flatMap.given
    import cats.syntax.functor.given
