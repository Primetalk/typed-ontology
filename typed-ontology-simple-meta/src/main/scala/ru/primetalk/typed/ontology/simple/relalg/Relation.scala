...
/** Relation is a pair of schema and a collection of instances of that schema. V - is the collection
  * type (List, Stream[...]).
  */
abstract class Relation[S <: RecordSchema, VS, V[_]](val schema: S)(using SchemaValueType[S, VS])
    extends ExprClassicDsl:
  self =>
  ...
  /** Строим декартово произведение отношения r1 и текущего отношения.
    */
  transparent inline def crossProductFrom[
      S1 <: RecordSchema,
      VS1,
      R1 <: Relation[S1, VS1, V],
      VRes
  ](r1: R1)(using
      fm: FlatMap[V],
  ...
  transparent inline def crossProduct[S2 <: RecordSchema, VS2, R2 <: Relation[S2, VS2, V], VRes](
      r2: R2
  )(using
      fm: FlatMap[V],
...
object Relation:
  transparent inline def apply[S1 <: RecordSchema, VS1, V[_]](
      s1: S1
  )(using svt1: SchemaValueType[S1, VS1])(inline v: V[svt1.Value]) =
    new Relation[S1, VS1, V](s1)(using svt1) {
      type Schema = S1
      val rows = v.asInstanceOf[V[VS1]]
    }

  transparent inline def empty[S1 <: RecordSchema, VS1, V[_]](s1: S1)(using svt1: SchemaValueType[S1, VS1])(using MonoidK[V]) =
    apply[S1, VS1, V](s1)(using svt1)(MonoidK[V].empty)
...
extension (tb: TableBuilder)
  transparent inline def relation[TSV, V[_]](using
      rsvt: SchemaValueType[tb.TableSchema, TSV]
