...
/** Metainformation about property. Contains unique name (within the type) and type of the value.
  ...
  */
abstract class SimplePropertyId[-A, B](val name: String, val tpe: ClassTag[B])
...
def fieldsImpl[R](expr: Expr[Seq[RecordProperty[Record[R]]]], schemaExpr: Expr[RecordSchema[R]])(
    using r: Type[R]
)(using Quotes): Expr[RecordSchema[R]] =
  expr match
  ...
  def prepend[P <: RecordProperty[Record[R]]](p: P) =
    new RecordSchema[R]:
      ...
      def get[A, P2 <: SimplePropertyId[Record[R], A]](p2: P2)(v: Values): Option[A] =
        v match
          case head *: tail =>
            if p2 == p then Some(head.asInstanceOf[A])
...
extension [D <: Tuple](t: D)
  def get[T, R, P <: SimplePropertyId[Record[R], T]](p: P)(using schema: RecordSchema[R])(using
      ev: D =:= schema.Values
  ): Option[T] =
    schema.get(p)(ev(t))
/*
