...
/** Иерархия типов, поддерживаемых онтологией.
  ...
  *
  * Запись/Record - тип, к которому могут быть привязаны значения свойств. Вполне соответствует
  * обычному объекту, или строчке в таблице БД.
  *
  * Скаляр/Scalar - обыкновенный тип, не имеющий внутренней структуры. Может только иметь значение.
  *
  * Коллекция/MetaSeq - обыкновенная коллекция наподобие Seq/List/Vector. Отличие от Scalar[Seq[T]]
  * заключается в том, что на мета-уровне можно будет привязать дополнительные ограничения на
  * количество элементов.
  *
  * Тип суммы/Sum- к нему можно привязать в онтологии исчерпывающий набор вариантов. Примерно
  * соответствует sealed trait.
  *
  * Тип произведения/Product - к ним можно привязать в онтологии жёсткий перечень вложенных
  * элементов. Примерно соответствует Tuple.
  */
