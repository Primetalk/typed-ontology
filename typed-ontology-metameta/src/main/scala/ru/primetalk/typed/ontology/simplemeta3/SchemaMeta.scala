...
sealed trait RecordSchema:
  ...
  transparent inline def indexOfProp[This >: this.type <: RecordSchema, P2 <: PropertySet](
      p2: P2
  ): IndexOfProp[p2.type]
  ...
  type IndicesOfProps[S2 <: RecordSchema] <: Tuple =
    S2 match
      ...
      case SchemaCons[p, s] =>
        IndexOfProp[p] *: IndicesOfProps[s]

  transparent inline def indicesOfProps[This >: this.type <: RecordSchema, S2 <: RecordSchema](
      s2: S2
  ): IndicesOfProps[s2.type] =
  ...
  transparent inline def concat[This >: this.type <: RecordSchema, S2 <: RecordSchema](
      schema2: S2
  ): RecordSchema.Concat[This, schema2.type] =
    inline this match
      case _: EmptySchema =>
        schema2
  ...
  transparent inline def concatValues[This >: this.type <: RecordSchema, S2 <: RecordSchema](
      schema2: S2
  ): (Values, schema2.Values) => RecordSchema.Concat[This, S2] =
    ???
  ...
  transparent inline def propertyGetter[This >: this.type <: RecordSchema, P](
      p: P
  ): PropertyGetter[p.type]
  ...
  transparent inline def projectorFrom[This >: this.type <: RecordSchema, S1 <: RecordSchema](
      s1: S1
  ): FromOtherValuesGetter[s1.Values]

object Converter:
  transparent inline def converter[S1 <: RecordSchema, S2 <: RecordSchema](s1: S1, s2: S2)(
      indices: s1.IndicesOfProps[S2]
  ): s1.Values => s2.Values =
    (v: s1.Values) =>
...
final case class SchemaCons[P <: RecordProperty0, S <: RecordSchema](p: P, schema: S)
  ...
  def get[P2 <: RecordProperty0](p2: P2)(v: Values): Option[p2.P] =
    v match
      case head *: (tail: schema.Values) =>
        if p2 == p then Some(head.asInstanceOf[p2.P])
  ...
  def unapply[This >: this.type <: SchemaCons[P, S]]: Unapply[This] =
    Some((p, schema))
  transparent inline def indexOfProp[This >: this.type <: RecordSchema, P2](
      p2: P2
  ): IndexOfProp[p2.type] =
    RecordSchema.indexOfProp(this, p2)
  ...
  transparent inline def propertyGetter[This >: this.type <: RecordSchema, P](
    ...
    val i = indexOfProp(p)
    _.apply(i).asInstanceOf[RecordProperty0.PropertyValueType[
      p.type
    ]] //// Tuple.Elem[Values, IndexOfProp[p.type]]]

  transparent inline def projection[S2 <: RecordSchema](schema2: S2)(using
      ev: schema2.PropertySet <:< PropertySet
  ): Values => schema2.Values =
...
object RecordSchema:
  ...
  transparent inline def valueAt[X <: RecordSchema, I <: Int](schema: X, inline i: I)(
      v: schema.Values
  ): ValueAt[X, I] =
  ...
  transparent inline def indexOfProp[S1 <: RecordSchema, P](
      schema: S1,
      inline property: P
...
trait SchemaBuilder extends RecordSchemaBuilderBase:
  ...
  transparent inline def fields2[P1 <: RecordProperty0, P2 <: RecordProperty0](
      inline p1: P1,
      inline p2: P2
  ) =
    sprepend(RecordSchema.empty, p2).prepend(p1)
  transparent inline def fields3[
      P1 <: RecordProperty0,
      P2 <: RecordProperty0,
      P3 <: RecordProperty0
  ](inline p1: P1, inline p2: P2, inline p3: P3) =
    sprepend(RecordSchema.empty, p3).prepend(p2).prepend(p1)

  transparent inline def tupleToSchema[T <: Tuple](inline t: T)(using
      inline ev: Tuple.IsMappedBy[RecordProperty][T]
  ): RecordSchema =
...
def tupleToSchemaImpl[T <: Tuple](t: Expr[T], ev: Expr[Tuple.IsMappedBy[RecordProperty][T]])(using
    Quotes
): Expr[RecordSchema] =
  t match
...
def propertyGetterImpl[V <: NonEmptyTuple, I <: Int](
    i: Expr[Int]
)(using vt: Type[V], rt: Type[I])(using Quotes): Expr[V => Tuple.Elem[V, I]] =
  '{
    // import quotes.reflect._
