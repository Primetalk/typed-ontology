...
trait TestDataStream extends BaseSpec:
  ...
  val orderItems = OrderItem.relation2(Stream.emits(Seq(orderItem1,orderItem2, orderItem3)))

class StreamSpec extends TestDataStream:

  given cats.Foldable[[T] =>> Stream[Pure, T]] with
    def foldLeft[A, B](fa: Stream[Pure, A], b: B)(f: (B, A) => B): B = 
      fa.fold(b)(f).compile.toList.head

    def foldRight[A, B](fa: Stream[Pure, A], lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] = 
      fa.fold(lb)((b,a) => f(a,b)).compile.toList.head
  ...
  test("projection"){
    ...
    ids.rows.compile.toList should equal(List(Tuple(1), Tuple(2)))
  ...
  test("cross product from"){
    ...
    poi.rows.compile.toList should equal(List(
  ...
  test("cross product"){
    ...
    res.rows.compile.toList should equal(List(
      (1,1,1,1,"product1"), 
      (1,1,1,2,"product2"), 
      (2,1,1,1,"product1"), 
      (2,1,1,2,"product2"), 
  ...
  test("Filter"){
    ...
    p.rows.compile.toList should equal(List(orderItem1))
  }
  test("Union"){
    val p = orderItems ++ orderItems
    p.rows.compile.count should equal( orderItems.rows.compile.count * 2)
  }
  ...
  test("Expenses report with a schema-based groupMapReduceS"){
    ...
    result.rows.compile.toList should equal(List(
      ("product1", BigInt(10)),
      ("product2", BigInt(20)),
    ))
  }
