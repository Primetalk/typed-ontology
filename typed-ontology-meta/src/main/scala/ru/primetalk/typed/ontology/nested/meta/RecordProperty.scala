...
/** Type of record property identifier.
  * @tparam A
  *   Phantom type of record. It's not an ontology type, because only records can have named
  *   attributes.
  */
...
/** Metainformation about property. Contains unique name (within the type) and type of the value.
  ...
  *   attributes.
  */
abstract class AttributeId[A, B <: OntologyType](name1: String, val tpe: RttiProvider[B])
...
trait PropertiesBuilder extends RecordSchemaBuilderBase:
  ...
  abstract class column[T <: OntologyType](using RttiProvider[T])
      extends AttributeId[RecordType, T]("", summon):
