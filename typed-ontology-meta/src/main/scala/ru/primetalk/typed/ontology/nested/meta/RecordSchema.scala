...
sealed trait RecordSchema:
  ...
  transparent inline def concatValues[S2 <: RecordSchema](schema2: S2)(
      schema3: RecordSchema.Concat[this.type, schema2.type]
  ...
  extension (values: Values)
    ...
    transparent inline def updated[P <: RecordProperty0](p: P)(
        inline v: RecordProperty0.PropertyValueType[p.type]
    ) =
...
final case class SchemaCons[P <: RecordProperty0, S <: RecordSchema](p: P, schema: S)
  ...
  def get[P2](p2: P2)(v: Values): Option[RecordProperty0.PropertyValueType[p2.type]] =
    if p2 == p then Some(v.head.asInstanceOf[RecordProperty0.PropertyValueType[p2.type]])
    else schema.get(p2)(v.tail.asInstanceOf[schema.Values])
  def convertToMap(v: Values, m: Map[String, Any] = Map()): Map[String, Any] =
  ...
  transparent inline def propertyGetter[This >: this.type <: RecordSchema, P <: RecordProperty0](
      p2: P
  ): PropertyGetter[p2.type] = v =>
    inline v match
      case (head: PValue) *: (tail: schema.Values) =>
        inline if p == p2 then head.asInstanceOf[RecordProperty0.PropertyValueType[p2.type]]
        else schema.propertyGetter(p2)(tail)
      // val i = indexOfProp(p)
...
object RecordSchema:
  ...
  // DOESN'T WORK
  transparent inline def removeDoesntWork[P1 <: RecordProperty0, S <: RecordSchema](
      p1: P1,
      inline schema: S
  ): Remove[P1, S] =
    inline schema match
